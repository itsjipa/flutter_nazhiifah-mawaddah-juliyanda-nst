...
class _FormAdvanceState extends State<FormAdvance> {
  ...
  Widget build(BuildContext context) {
    final contactProvider = Provider.of<contact_store.Contacts>(context);
    return Scaffold(
      appBar: AppBar(
        title: const Text(
          "Contacts",
                ...
                child: ElevatedButton(
                  onPressed: () {
                    ...
                    if (_formKey.currentState!.validate()) {
                      bool existsContacts = contactProvider.contacts.any(
                          (contact) =>
                              contact['name'] == name ||
                              contact['nomor'] == nomor);
                      if (!existsContacts) {
                        setState(() {
                          contactProvider.addContacts({
                            'nama': name,
                            'nomor': nomor,
                            'date': _dueDate,
                            'color': _currentColor,
                        ...
                        contactProvider.contacts[selectIndex]['nama'] = name;
                        contactProvider.contacts[selectIndex]['nomor'] = nomor;
                      }
                  ...
                  itemBuilder: (context, index) {
                    return ListTile(
                      ...
                      leading: CircleAvatar(
                        child:
                            Text(contactProvider.contacts[index]['nama']![0]),
                      ),
                      title: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          // list nama
                          Text(
                            contactProvider.contacts[index]['nama']!,
